...
void FFMpegPlayer::openFile(const ffmpeg::DecodePar& p) {
    ...
    try {
        if (m_audioNumChannels > 0) {
            if (!p.portaudioEngine && !m_paudioIntern.init({
                .sampleRate = 48000,
                .numChannels = 2
            })) {
            ...
            m_paudio = p.portaudioEngine ? p.portaudioEngine : &m_paudioIntern;
            m_paudio->printInfo();
            ...
            bool useConverter = p.portaudioEngine != nullptr;
...
void FFMpegPlayer::recvAudioPacket(audioCbData& data) {
    if (!m_bufSizeFact) {
        ...
        if (std::fmod(f, 1.0) != 0.0) {
            LOGE << "FFMpegPlayer::recvAudioPacket error, packet sample size not a multiple of audio frameBuffer size!";
        }
