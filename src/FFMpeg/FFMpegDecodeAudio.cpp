...
void FFMpegDecodeAudio::recv_audio_packet(audioCbData& data)
{
    ...
    if (!m_paudio.getCycleBuffer()) return;
    ...
    if (m_paudio.getCycleBuffer()->empty())
    {
        ...
        {
            if (m_paudio.isRunning()) m_paudio.pause();
            m_paudio.setFramesPerBuffer(data.samples);
        }

        // how manu portaudio buffers fit in the ffmpeg audio buffer?
        m_bufSizeFact = std::max<size_t>(data.samples / m_paudio.getFramesPerBuffer(), 1);

        // register this multiplication factor in order to know when to free the feed block again
    ...
    if (m_paudio.getCycleBuffer()->size())
    {
        if ((AVSampleFormat)data.sampleFmt != AV_SAMPLE_FMT_FLT)
        {
