...
static int encode_write_frame(unsigned int stream_index, int flush)
{
    ...
    AVFrame *filt_frame = flush ? nullptr : filter->filtered_frame;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        if ((ret = open_input_file(argv[1])) < 0) {
            throw std::runtime_error("open_input_file failed");
        }
        if ((ret = open_output_file(argv[2])) < 0) {
            throw std::runtime_error("open_output_file failed");
        }
        ...
        for (int i = 0; i < ifmt_ctx->nb_streams; i++) {
            ...
            av_log(nullptr, AV_LOG_INFO, "Flushing stream %u decoder\n", i);
